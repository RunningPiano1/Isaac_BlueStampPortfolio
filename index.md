...
I am so happy to be have put on the finishing touches and modifications on my thermal camera project. This week was mostly spent on preparing for demo night on Thursday but it was accompanied with moments of creativity. In addition to fixing a few bugs here and there, my two main modifications to wrap up the project for the **active buzzer** used for overheating detection and a **distance-from-object** display on matplotlib. The active buzzer was the simpler of the two modifications as it only involved plugging the buzzer onto a breadboard and linking it with the Raspberry Pi with two male-to-female wires. Although I constructed my circuit in unconventional fashion, the buzzer works as normal and poses the danger to the user. The distance-from-object display modifications was more complicated because I was originally considering using formulas I found online to approximate the distance between the sensor itself and the object it is facing. However, I eventually decided on using a ultrasonic sensor that could measure distances more accurately. The ultrasonic sensor is called the HC-SR04 and, in addition to using the GND and VCC 5V power pins, also uses the "Trig" and "Echo" pins that use Pin 11 and Pin 12, or GPIO-17 and GPIO-18, on the Raspberry Pi respectively. The Trig pin is responsible for emitting ultrasonic pulses out into the surrounding area. The Echo pin is responsible for receiving the reflected pulse and outputs a signal duration. Since the MLX90640 and HC-SR04 are two separate devices, I needed to combine them both into one body. I did this by cutting off some parts of the "blister pack" that originally held the HR-SR04s in the packaging. I then used the piece I cut off as a mold that could hold both the MLX90640 and the HR-SR04 in one place, together as close as possible to each other. That way, the distance measurements that came from the the HR-SR04 would be accurate to the actual distance between the MLX90640 and the object. 

Over the three week period at BlueStamp, my biggest challenge was installing the neceessary modules and packages to make programming and displaying possible. This was a huge issue early on in project development because the tutorial technically never explicitly listed out the different applications and settings I needed to integrate the Raspberry Pi and MLX90640 on the software level. At first glance, connecting the correct pins between the sensor and the Pi appeared to be enough but I actually had to install the individual libraries and modules on my own so that I could program, display and enable remote machine connection. That being said, my biggest triumph was getting the display on my monitor. Given the numerous complications and challenges I had to undergo prior to getting to that point, it seemed near impossible that once I was confident that the system was fully set up, the heatmap would display on my first try. But miraculously, the code worked instantly and my heatmap appeared on my screen, complete with the colorbar, axes, and temperature readings.

After everything I have learned at BlueStamp Engineering, I hope to go deeper into areas that will help me develop ideas to transform my project(s) into real-life applicable tools. To me, it is important not only to complete projects for the sake of finishing a program like BlueStamp, but also to create tools that have personal and practical value. I also hope to strengthen my understanding of signal processing and data analysis, since being able to filter and interpret sensor data more effectively would open up applications. Finally, I want to continue engaging in programs similar to BlueStamp that will help build my design and innovation skills, so I can take an idea from concept all the way to a finished product.
